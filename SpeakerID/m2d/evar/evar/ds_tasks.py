...
def get_defs(cfg, task):
    ...
    # task_metadata, task_data, n_folds, unit_sec, activation, balanced
    # task_metadata is CSV containing file_name, label, speaker_id, split (train/valid/test)
    return (
        f'{METADATA_DIR}/{task}.csv',
        f'{WORK}/{_fs_table[cfg.sample_rate]}/{folder}',
        folds,
        unit_sec,
        activation,
        balanced
